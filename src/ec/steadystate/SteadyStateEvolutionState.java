...
/**
 * This subclass of EvolutionState implements basic Steady-DecisionProcessState Evolution and (in distributed form)
 * Asynchronous Evolution. The procedure is as follows.  We begin with an empty Population and one by
 * one create new Indivdiuals and send them off to be evaluated.  In basic Steady-DecisionProcessState Evolution the
 ...
 * one by one, and sent off to be evaluated.  Once again, in basic Steady-DecisionProcessState Evolution the
 * individuals are immediately evaluated and we wait for them; but in Asynchronous Evolution the individuals are evaluated
...
public class SteadyStateEvolutionState extends EvolutionState
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        if (!(breeder instanceof SteadyStateBreeder))
            state.output.error("You've chosen to use Steady-DecisionProcessState Evolution, but your breeder is not of the class SteadyStateBreeder.",base);
        if (!(evaluator instanceof SteadyStateEvaluator))
            state.output.error("You've chosen to use Steady-DecisionProcessState Evolution, but your evaluator is not of the class SteadyStateEvaluator.",base);
        if (!(exchanger instanceof SteadyStateExchangerForm))
            state.output.error("You've chosen to use Steady-DecisionProcessState Evolution, but your exchanger does not implement the SteadyStateExchangerForm.",base);
    ...
    void checkStatistics(final EvolutionState state, Statistics stat, final Parameter base)
        ...
        if (!(stat instanceof SteadyStateStatisticsForm))
            state.output.warning("You've chosen to use Steady-DecisionProcessState Evolution, but your statistics does not implement the SteadyStateStatisticsForm.",base);
